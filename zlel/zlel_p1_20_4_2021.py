...
class branch:
    ...
    def get_M_row(self, branch_list, Vi=0.6, Vce=0.6):
    ...
    def get_Us_row(self, t, Vi=0.6, Vbc=0.6):
        row = np.zeros(1)
        if self.name[0] == "R":
            pass
        elif self.name[0] == "V":
    ...
    def transistorValues(self, vbe, vbc):
        if self.name[0] != "Q":
            sys.exit("diod function was called by not diod element")
            raise(ValueError)
        Vt = 8.6173324e-5 * 300
...
def solve_orders(T, U, orders, branch_list, nd_list, el_num, Aa):
    b = len(branch_list)
    n = len(nd_list)
    cirLineal = True
    for branch in branch_list:
    ...
    for order in orders:
        ...
        if orderID == ".OP":
            if cirLineal:
                ...
                p2.print_solution(solveNonLinealOP(b, n), b, n)

        elif orderID == ".DC":
            solveDC(order, b, n, cirLineal)

        elif orderID == ".TR":
            solveTR(order, b, n, cirLineal)
...
def solveTR(order, b, n, cirLineal):
...
def solveNonLinealOP(b, n, t=0):
    ...
    currentT = T
    currentU = U
    currentSol = np.linalg.solve(currentT, currentU)
    iteration = 1
    max_iteration = 100
    while True:
...
if __name__ == "__main__":
    ...
    # solve orders
    solve_orders(T, U, orders, branch_list, nd_list, el_num, Aa)

    p2.plot_from_cvs("../cirs/all/2_zlel_arteztailea.tr", "t", "e2", "wololo")
